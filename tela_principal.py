...
class TelaMenu:
    ...
    def menu(self):
        self.tpl_menu = tk.Toplevel(self.janela) # Inicia uma nova tela a partir do botão da tela de login.
        self.tpl_menu.geometry('700x700')

        self.mnu_principal = tk.Menu(self.tpl_menu) # criando uma barra de menu que vai receber elementos como inventario, e outros. Atenção: em caso de mudança, tomar cuidado pois o menu só aparece na tela quanado com os seus elementos, seja um ou mais.

        # Adicionando o menu "Inventário" á barra:
        self.mnu_inventario = tk.Menu(self.mnu_principal, tearoff=0) # gerando um "botão" na barra de menu
        ...
        # Frame do Inventário:
        self.frm_inventario = tk.Frame(self.tpl_menu) # Criando o frame na tela tpl_menu. Ele está na tela menu, mas os componentes de dentro do frame apenas aparecem quando o usuário interagir com o menu.
    ...
    def interacao_inventario(self): # inicia uma função para mostrar o frame apenas ao interagir com o menu. Cada um dos elementos do menu precisa de um desses para serem independentes(até onde eu sei).
        ...
        self.lbl_ad_arquivo = tk.Label(self.frm_inventario, text= 'Adicione o Arquivo') # label
        self.lbl_ad_arquivo.pack() # mostrando a label

        self.btn_ad_arquivo = tk.Button(self.frm_inventario, text= 'Upload', command= self.upp_arquivo) # botão com comando para carregar um arquivo da máquina
